...
def generate_certificate(documentName, recipientEmail, orgEmail, cpi, output_path, institute_logo_path=None):
    ...
    institute = Paragraph(orgEmail, institute_style)
    ...
    title1 = Paragraph(documentName, title_style)
    ...
    recipient_text = f"This is to certify that<br/><br/>\
                     <font color='red'> {recipientEmail} </font><br/>\
                     with CPI <br/> \
                    <font color='red'> {cpi} </font> <br/><br/>\
                     has successfully was attending:<br/>\
                     <font color='blue'> {orgEmail} </font>"
